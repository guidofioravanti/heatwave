...
dashboardHeader(title = "Heatwave Tracker")->header
...
body<-dashboardBody(
  fluidPage(id="panel1",
            ...
            absolutePanel(
              top="75px",
              left="25px",
              shinyWidgets::sliderTextInput(inputId = "year",label="Year",choices = c(2003,2010,2015),grid = TRUE,force_edges = TRUE),
              shinyWidgets::sliderTextInput(inputId = "month",label = "Month",choices = month.name[5:9],grid=TRUE,force_edges = TRUE),
              sliderInput(inputId = "day",label = "Day",min = 1,max = 153,value = 1,step = 1,animate = TRUE),
