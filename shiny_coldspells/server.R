...
colorBin(palette=scico::scico(n=7,palette = "imola"),
         domain = c(0,100),
...
server<-function(input,output){
  ...
  calendario<-reactive({ seq.Date(from = as.Date(glue::glue("{input$year}-01-01")),to=as.Date(glue::glue("{input$year}-05-31")),by="day") })
  
  reactive({
    
    brick(glue::glue("hw_{input$year}-01-01_{input$year}-05-31.tif"))->hw
    hw[hw<3]<-NA
    names(hw)<-calendario()
    
    brick(glue::glue("i2n_{input$year}-01-01_{input$year}-05-31.tif"))->i2n
    i2n[is.na(hw)]<-NA
    i2n[i2n>30]<-30
    crs(i2n)<-crs(hw)
  ...
  #     as.numeric(raster::extract(mappa()[[c((input$day-2):(input$day+2))]],df))->valori
  #     valori[is.na(valori)]<-0
  #     dfPlot<-data.frame(x=1:length(valori),y=round(valori,1))
  #     
  #     leafletProxy(mapId="map_hw") %>%
